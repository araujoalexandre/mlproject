...
def main():
    ...
    # validation
    validation = pickle_load("{}/validation.pkl".format(folder_path))
    ...
    for enum, model in enumerate(models_to_run):
        ...
        if os.path.exists("{}/Test/".format(folder_path)):
            ...
            for fold in range(n_folds):
                ...
                if 'xgboost' in str(model_test):
                    tree = model_test.best_ntree_limit
                    predict_test = model_test.predict(X_test, ntree_limit=tree)
                else:
            ...
            if do_submit and id_test is not None:
